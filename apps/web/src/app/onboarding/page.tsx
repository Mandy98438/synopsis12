  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-16">
        ...
        {step === "verify" ? (
          <VerifyStep connecting={connecting} onConnect={handleLinkedIn} onSkip={handleSkip} />
        ) : (
...
function VerifyStep({
  ...
  return (
    <div className="w-full max-w-[400px]">
      <p className="text-[#333] text-xs tracking-widest uppercase text-center mb-8">Step 1 of 2</p>
        ...
        <div className="relative">
          ...
          <div className="absolute -bottom-1 -right-1 flex gap-1">
            {PLATFORM_BADGES.map((p) => (
              <div key={p.label} className="w-5 h-5 rounded-full flex items-center justify-center text-[7px] font-bold text-white border border-[#0a0a0a]" style={{ background: p.bg }}>
                {p.short}
              </div>
            ))}
          </div>
      ...
      <h1 className="text-2xl font-medium text-white text-center mb-3">Verify your identity</h1>
      ...
      <button onClick={onConnect} disabled={connecting} className="w-full py-3.5 rounded-xl bg-[#0077B5] hover:bg-[#006097] text-white text-sm font-medium transition-colors disabled:opacity-50 mb-3 flex items-center justify-center gap-2">
...
function ReadyStep({ onBuild }: { onBuild: () => void }) {
  return (
    <div className="w-full max-w-[400px] text-center">
      <p className="text-[#333] text-xs tracking-widest uppercase mb-8">Step 2 of 2</p>
      ...
      <button onClick={onBuild} className="w-full py-3.5 rounded-xl bg-[#E07020] hover:bg-[#c85e18] text-white text-sm font-medium transition-colors">
        Build my Kard →
      </button>
    </div>
  );
